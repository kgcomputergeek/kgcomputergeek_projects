...
import random  # Import the random module to randomly select cards
...
def deal_card():
    ...
    card = random.choice(cards)  # Randomly choose a card from the deck
    return card  # Return the chosen card
...
def calculate_score(cards):
    ...
    if 11 in cards and sum(cards) > 21:  # Check if there's an ace and the score is over 21
        cards.remove(11)  # Replace the ace value from 11 to 1
        cards.append(1)  # Add 1 to the hand
    return sum(cards)  # Return the total score of the hand

# Define a function to compare the user's score with the computer's score
...
# Define the main function to play the Blackjack game
def play_game():
    print(logo)  # Print the game logo at the start

    # Deal initial cards to the user and computer
    user_cards = [deal_card(), deal_card()]
    computer_cards = [deal_card(), deal_card()]
    
    # Set a flag to indicate whether the game should continue
    is_game_over = False
    ...
    # Let the computer play until it reaches a score of at least 17
